...
contract EncryptedUpdateRegistry {
    struct Record {
        address submitter;
        string signerId;
        string kind;
    ...
    event EncryptedLocalUpdateSubmitted(
    ...
    function submitEncryptedLocalUpdate(
        bytes32 roundContextDigest,
        ...
        emit EncryptedLocalUpdateSubmitted(
            roundContextDigest, msg.sender, transactionHash, payloadDigest, timestampNs
        );
        return transactionHash;
    }

    function getEncryptedLocalUpdates(bytes32 roundContextDigest)
        external
        view
